...
const sections=entries.map(([key,value],index)=>({id:index+1,key,stage:value.appearance[0]-1,group:value.appearance[1],spell:!!value.spell,bgm:value.bgm,
 names:Array.from({length:5},(_,difficulty)=>{const selector='ENHLX'[difficulty];const entry=Object.entries(value).find(([k])=>k.startsWith('!')&&k.includes(selector))?.[1];return typeof entry==='string'?glossary[entry]||[entry,entry,entry]:entry||[key,key,key];})}));
const files={

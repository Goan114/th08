...
export async function readJsonLinesArray(path) {
  const rows = [];
  for await (const row of readJsonLinesMaybeGzip(path)) rows.push(row);
  return rows;
}
...
export async function loadGoldenManifest(goldenRoot = defaultGoldenRoot, {verify = true} = {}) {
  ...
  if (manifest.schema !== 'eagler/replay-golden-set/v1' || manifest.game !== 'th08')
    throw Error('Unsupported TH08 golden manifest');

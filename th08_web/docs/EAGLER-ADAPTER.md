...
- DATA: the parent's managed retail-memory lease, mounted as
  `/game/th08.dat`. The Japanese font is the canonical shared Launcher
  `/msgothic.ttc`, not a duplicated Runtime asset. The adapter symlinks it to
  `/fonts/msgothic.ttc` for the existing SDL binary. The Runtime only packages
  its small text-support tables (`blend.bin` and `cp932.bin`).
- Music: canonical `/bgm-ogg/th08_*.ogg`; stream and full decode are native.

...
document.addEventListener('visibilitychange',()=>{if(!core||!launched)return;core.sdl_keys_clear();cancelTouches();if(document.hidden){core.sdl_loop_pause(1);queue=queue.then(persist).catch(error);}else void resumeForegroundAudio(true);});
window.addEventListener('blur',()=>{if(core){practice?.clear();core.sdl_keys_clear();cancelTouches();}});
window.addEventListener('pagehide',()=>{cancelTouches();if(core&&launched){core.sdl_loop_pause(1);void persist().catch(console.error);}});

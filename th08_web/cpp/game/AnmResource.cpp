...
bool AnmResource::configure_texture(u32 index,u32 handle,u32 width,u32 height){
    ...
    const float sx=(Extended::from_int64(width)/Extended::from_int64(source.width)).to_float(),sy=(Extended::from_int64(height)/Extended::from_int64(source.height)).to_float();
    for(u32 i=source.first_sprite;i<source.first_sprite+source.sprite_count;++i){const auto& raw=sprite_sources[i];auto& sprite=sprites[i];sprite={};sprite.anmIdx=loaded.anmIdx;sprite.texture=handle;sprite.width=width;sprite.height=height;sprite.scaleFactor={sx,sy};
        ...
        sprite.endPixelInclusive={((number(raw.x)+number(raw.width))*number(sx)).to_float(),((number(raw.y)+number(raw.height))*number(sy)).to_float()};
        ...
        sprite.widthPx=((number(sprite.endPixelInclusive.x)-number(sprite.startPixelInclusive.x))/number(sx)).to_float();sprite.heightPx=((number(sprite.endPixelInclusive.y)-number(sprite.startPixelInclusive.y))/number(sy)).to_float();
    }
    return true;
}

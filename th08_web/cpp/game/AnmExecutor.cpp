...
float mix(float from,float to,float t){return (number(t)*(number(to)-number(from))+number(from)).to_float();}
u8 mix_byte(u8 from,u8 to,float t){return u8((number(t)*(Extended::from_int(to)-Extended::from_int(from))+Extended::from_int(from)).truncate_int());}
...
struct Arguments {
    AnmVm& vm;AnmRawInstr* instruction;
    ...
    float fv(u32 n) const {return instruction->varMask&(1u<<n)?vm.GetFloatVar(f(n)).to_float():f(n);}
...
Extended AnmVm::GetFloatVar(float id){const i32 n=Scalar::truncate(id);if(auto* p=integer(*this,n))return Extended::from_int(*p);if(auto* p=floating(*this,n))return number(*p);return number(id);}
...
i32 AnmLoaded::SetSprite(AnmVm* vm,i32 index) noexcept {
    ...
    vm->matrix3.m[0][0]=(number(s.widthPx)/number(s.width)*number(s.scaleFactor.x)).to_float();
    vm->matrix3.m[1][1]=(number(s.heightPx)/number(s.height)*number(s.scaleFactor.y)).to_float();
...
bool AnmExecutor::execute(AnmVm& vm){
    ...
    for(;;){
        ...
        const Arguments a{vm,instruction};const i32 op=instruction->opcode;
        ...
        case 46:*a.fp(0)=(number(*a.fp(0))/number(a.fv(1))).to_float();break;
        case 48:*a.fp(0)=float_remainder(a.fv(0),a.fv(1)).to_float();break;
        ...
        case 56:*a.fp(0)=(number(a.fv(1))/number(a.fv(2))).to_float();break;
        case 58:*a.fp(0)=float_remainder(a.fv(1),a.fv(2)).to_float();break;
        case 59:*a.ip(0)=signed_bits(rng.bounded32(u32(a.iv(1))));break;
        case 60:*a.fp(0)=rng.range(a.fv(1)).to_float();break;
        case 61:*a.fp(0)=sine(a.fv(1)).to_float();break;
        case 62:*a.fp(0)=cosine(a.fv(1)).to_float();break;
        case 63:*a.fp(0)=tangent(a.fv(1)).to_float();break;
        case 64:*a.fp(0)=arccosine(a.fv(1)).to_float();break;
        case 65:*a.fp(0)=arctangent(a.fv(1)).to_float();break;
...
void AnmExecutor::advance(AnmVm& vm){
    ...
    for(u32 i=0;i<7;++i)if(vm.interpEndTimers[i].current>0){
        ...
        else weight=(number(vm.interpCurrentTimers[i].value().to_float())/vm.interpEndTimers[i].value()).to_float();
        switch(vm.interpModes[i]){
        case 1:weight=mul(weight,weight);break;
        case 2:weight=(number(weight)*number(weight)*number(weight)).to_float();break;
        // Original FST stores a rounded float while retaining the extended
        // intermediate used by the following multiplication (0x4619a4..0x461a19).
        case 3:{const auto square=number(weight)*number(weight);weight=(square*number(square.to_float())).to_float();break;}
        case 4:{const auto inverse=number(1)-number(weight);weight=(inverse*number(inverse.to_float())).to_float();weight=sub(1,weight);break;}
        case 5:{const auto inverse=number(1)-number(weight);const auto rounded=number(inverse.to_float());weight=(inverse*rounded*rounded).to_float();weight=sub(1,weight);break;}
        case 6:{const auto inverse=number(1)-number(weight);const auto square=inverse*number(inverse.to_float());weight=(square*number(square.to_float())).to_float();weight=sub(1,weight);break;}
        default:break;
        }
        switch(i){
        ...
        case 3:vm.rotation={add_angle(((number(vm.rotateFinal.x)-number(vm.rotateInitial.x))*number(weight)).to_float(),vm.rotateInitial.x),add_angle(((number(vm.rotateFinal.y)-number(vm.rotateInitial.y))*number(weight)).to_float(),vm.rotateInitial.y),add_angle(((number(vm.rotateFinal.z)-number(vm.rotateInitial.z))*number(weight)).to_float(),vm.rotateInitial.z)};vm.updateRotation=true;break;
    ...
    if(vm.scaleGrowth.y!=0){vm.scale.y=(number(vm.scale.y)+number(timing.rate)*number(vm.scaleGrowth.y)).to_float();vm.updateScale=true;}
    if(vm.scaleGrowth.x!=0){vm.scale.x=(number(vm.scale.x)+number(timing.rate)*number(vm.scaleGrowth.x)).to_float();vm.updateScale=true;vm.updateRotation=true;}

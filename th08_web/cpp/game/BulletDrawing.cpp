...
void BulletDrawing::bullet(BulletState& b,const Vec2& origin){
    ...
    if(presentation::active){const size_t index=size_t(&b-state.bullets);const auto& p=previous_bullets[index];if(p.active&&p.state==b.state&&p.script==source.scriptIndex&&p.sprite==source.activeSpriteIndex&&b.active_time.current>=p.age&&close_enough(p.position,b.position)){position=presentation_lerp(p.position,b.position);angle=presentation_angle(p.angle,b.angle);}}
...
void BulletDrawing::laser(LaserState& l,const Vec2& origin){
    ...
    AnmVm body_copy,cap_copy;AnmVm* body=&l.animation[0];if(presentation::render_only){body_copy=*body;body=&body_copy;}body->pos=position(midpoint,.06f);if(smooth_laser){body->scale.x=scale_x;body->scale.y=scale_y;body->updateScale=true;}if(!presentation::render_only)l.color=-1;offset(*body);actions.draw(*body);

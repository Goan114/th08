...
void BulletDrawing::snapshot(){
    ...
    for(size_t i=0;i<previous_lasers.size();++i){const auto& l=state.lasers[i];auto& p=previous_lasers[i];p.active=l.in_use!=0;if(p.active){p.position=l.position;p.angle=l.angle;p.start_offset=l.start_offset;p.end_offset=l.end_offset;p.age=l.timer.current;}}
}
...
void BulletDrawing::laser(LaserState& l,const Vec2& origin){
    ...
    AnmVm body_copy,cap_copy;AnmVm* body=&l.animation[0];if(presentation::render_only){body_copy=*body;body=&body_copy;}body->pos=position(midpoint,.06f);if(!presentation::render_only)l.color=-1;offset(*body);actions.draw(*body);
